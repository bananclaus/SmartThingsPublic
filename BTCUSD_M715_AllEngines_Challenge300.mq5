//+------------------------------------------------------------------+
//| BTCUSD_M715_AllEngines_Challenge300.mq5                         |
//| M715: Tighter entries + $10 step lock + centered dashboard
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M715 AllEngines";
input int InpMagicNumber                    = 715;
...
int OnInit()
{
   ...
   {
      Print("M715 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M715 ready. Tight entry + $10 step lock.";
   ShowDashboard();
...
{
   return "M715_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M715_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   string lines[];
   ArrayResize(lines, 8);
   lines[0] = "=== BTCUSD M715 Tight Entry + $10 Lock ===";
   lines[1] = "Lot: " + DoubleToString(NormalizeLots(InpLots), 2) + " | Max loss: $" + DoubleToString(InpDailyMaxLossUSD, 2);
   lines[2] = "EA P/L: $" + DoubleToString(dayProfit, 2) + " | Trades: " + IntegerToString(todayTrades) + "/" + IntegerToString(InpMaxTradesPerDay) + " | Losses: " + IntegerToString(todayLosses) + "/" + IntegerToString(InpMaxLossesPerDay);
   lines[3] = "Spread: " + IntegerToString(GetSpreadPoints()) + "/" + IntegerToString(InpMaxSpreadPoints) + " | Session: " + (IsTradingSession() ? "OPEN" : "CLOSED");
   lines[4] = "BE: $" + DoubleToString(InpBE_StartUSD, 1) + " lock $" + DoubleToString(InpBE_LockUSD, 1) + " | Trail: $" + DoubleToString(InpTrailDistanceMoneyUSD, 1) + " | Step: $" + DoubleToString(InpProfitStepUSD, 0) + " | Streak: " + IntegerToString(consecutiveLosses);
   lines[5] = lastStepLockInfo;
   lines[6] = "Status: " + lastStatus;
   lines[7] = "Path: " + lastEntryPath + " | Signal: " + lastSignal;

   const int chartH = (int)ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);
   const int startY = chartH / 2 - 60;
   const int lineSpacing = 16;

   for(int i = 0; i < ArraySize(lines); i++)
   {
      const string name = "M715_DASH_" + IntegerToString(i);
      if(ObjectFind(0, name) < 0)
      {
         ObjectCreate(0, name, OBJ_LABEL, 0, 0, 0);
         ObjectSetInteger(0, name, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ...
      ObjectSetInteger(0, name, OBJPROP_COLOR, i == 0 ? clrDodgerBlue : clrWhite);
   }
   Comment("");
}
//+------------------------------------------------------------------+
