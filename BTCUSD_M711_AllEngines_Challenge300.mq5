...
input double InpAtrTPMult                   = 2.2;
input double InpMinTP_MoneyUSD              = 10.0;
input double InpMinRewardRisk               = 1.20;
...
input double InpTrailStartMoneyUSD          = 5.0;
input double InpTrailDistanceMoneyUSD       = 3.0;
input double InpMinSLModifyMoneyUSD         = 2.0;
input bool InpShowTrailOnChart              = true;

input bool InpUseProfitStepLock             = true;
input double InpProfitStepUSD                = 5.0;
...
input double InpImpulseBodyAtr              = 0.45;
input double InpStrongImpulseBodyAtr        = 0.90;
input double InpMaxWickBodyRatio            = 1.20;
input bool InpEnforceTrendAlign             = false;
input bool InpUseTrendFilter                = false;
input bool InpRequireM5TrendAlign           = false;
input bool InpRequireM1TrendAlign           = false;
...
input double InpSpikeRangeAtrMult           = 0.55;
input double InpSpikeBodyAtrMult            = 0.22;
input double InpPredictRangeAtrMult         = 0.32;
input double InpPredictBodyAtrMult          = 0.15;
input double InpSpikeMaxWickRatio           = 1.05;

input bool InpUseMomoBreak                  = true;
input double InpMomoRangeAtrMult            = 0.30;
input double InpMomoBodyAtrMult             = 0.12;
...
input double InpRSIBurstBodyAtr             = 0.55;
...
input double InpDualM5BodyAtr               = 0.90;
input double InpDualM1BodyAtr               = 0.35;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(InpMaxHoldMinutes > 0 && TimeCurrent() - openTime >= InpMaxHoldMinutes * 60)
      {
         if(profit >= 0.0)
         {
            trade.PositionClose(ticket);
            RemoveChartTradeLines(ticket);
            lastStatus = StringFormat("Closed: max hold time (profit $%.2f).", profit);
            continue;
         }
         else
         {
            lastStatus = StringFormat("Max hold reached but in loss ($%.2f) - holding for BE.", profit);
         }
      }
      ...
      {
         const double trailSL = isBuy ? currentPrice - trailDist : currentPrice + trailDist;
         if(isBuy)
         {
