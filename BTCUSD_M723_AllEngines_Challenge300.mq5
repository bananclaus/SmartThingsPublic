//+------------------------------------------------------------------+
//| BTCUSD_M723_AllEngines_Challenge300.mq5                         |
//| M723: M722 + back to $35 SL (BTC needs room)
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M723 AllEngines";
input int InpMagicNumber                    = 723;
...
int OnInit()
{
   ...
   {
      Print("M723 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M723 ready. M722 + $35 SL.";
   ShowDashboard();
...
{
   return "M723_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M723_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M723 $35SL+Spike+FastBE ===\n";
