//+------------------------------------------------------------------+
//| BTCUSD_M720_AllEngines_Challenge300.mq5                         |
//| M720: M713 base + early entry filter + tuned trailing
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M720 AllEngines";
input int InpMagicNumber                    = 720;
...
input double InpMaxBuyRsi                   = 52.0;
input double InpMinSellRsi                  = 48.0;

input bool InpUseSpikePredict               = true;
input bool InpUseSpikeConfirm               = true;
input double InpSpikeRangeAtrMult           = 0.75;
input double InpSpikeBodyAtrMult            = 0.35;
input double InpPredictRangeAtrMult         = 0.50;
input double InpPredictBodyAtrMult          = 0.25;
input double InpSpikeMaxWickRatio           = 0.75;

input bool InpUseMomoBreak                  = true;
input double InpMomoRangeAtrMult            = 0.55;
input double InpMomoBodyAtrMult             = 0.30;
...
int OnInit()
{
   ...
   {
      Print("M720 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M720 ready. M713 base + early filter + wide trail.";
   ShowDashboard();
...
void TryOpenTrade()
{
   ...
   {
      lastStatus = "Blocked: entry too late – move exhausted.";
      return;
   }
...
bool IsEntryTooLate(const int direction, const double atrValue)
{
   ...
   if(pctUsed > InpMaxCandleUsedPct && range > atrValue * 0.4)
      return true;
...
{
   return "M720_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M720_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M720 Base713 + EarlyFilter ===\n";
