//+------------------------------------------------------------------+
//| BTCUSD_M717_AllEngines_Challenge300.mq5                         |
//| M717: Early entry filter – no late entries after move is done
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M717 AllEngines";
input int InpMagicNumber                    = 717;
...
input int InpCooldownAfterLossMinutes       = 2;
input int InpMinMsBetweenEntries            = 30000;
...
input double InpBE_StartUSD                 = 2.0;
input double InpBE_LockUSD                  = 0.5;
input bool InpUseTrailing                   = true;
input double InpTrailStartMoneyUSD          = 6.0;
input double InpTrailDistanceMoneyUSD       = 4.0;
...
input double InpImpulseBodyAtr              = 0.85;
input double InpStrongImpulseBodyAtr        = 1.30;
input double InpMaxWickBodyRatio            = 0.60;
...
input double InpSpikeRangeAtrMult           = 1.00;
input double InpSpikeBodyAtrMult            = 0.50;
input double InpPredictRangeAtrMult         = 0.70;
input double InpPredictBodyAtrMult          = 0.40;
input double InpSpikeMaxWickRatio           = 0.50;

input bool InpUseMomoBreak                  = true;
input double InpMomoRangeAtrMult            = 0.80;
input double InpMomoBodyAtrMult             = 0.50;

input bool InpUsePullback                   = false;
input bool InpUseLivePullback               = false;
...
input double InpRSIBurstBodyAtr             = 0.95;
...
input double InpDualM5BodyAtr               = 1.30;
input double InpDualM1BodyAtr               = 0.65;
...
int OnInit()
{
   ...
   {
      Print("M717 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M717 ready. Early entry filter active.";
   ShowDashboard();
...
{
   return "M717_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M717_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M717 Early Entry Filter ===\n";
