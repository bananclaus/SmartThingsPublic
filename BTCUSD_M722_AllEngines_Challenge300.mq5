//+------------------------------------------------------------------+
//| BTCUSD_M722_AllEngines_Challenge300.mq5                         |
//| M722: M721 + allow strong spikes against trend
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M722 AllEngines";
input int InpMagicNumber                    = 722;
...
input double InpHardMaxLossUSD              = 20.0;
input double InpTargetSL_MoneyUSD           = 20.0;
input double InpMinSL_MoneyUSD              = 20.0;
...
input double InpAtrSLMult                   = 0.40;
input double InpAtrTPMult                   = 3.5;
input double InpMinTP_MoneyUSD              = 20.0;
input double InpMinRewardRisk               = 2.50;
input int InpMaxHoldMinutes                 = 45;

input bool InpUseBreakEven                  = true;
input double InpBE_StartUSD                 = 1.0;
input double InpBE_LockUSD                  = 0.2;
input bool InpUseTrailing                   = true;
input double InpTrailStartMoneyUSD          = 12.0;
input double InpTrailDistanceMoneyUSD       = 7.0;
input double InpMinSLModifyMoneyUSD         = 2.0;
...
input double InpImpulseBodyAtr              = 0.55;
input double InpStrongImpulseBodyAtr        = 1.00;
input double InpMaxWickBodyRatio            = 0.90;
...
input double InpBuyRsiMin                   = 35.0;
input double InpSellRsiMax                  = 65.0;
input double InpMaxBuyRsi                   = 55.0;
input double InpMinSellRsi                  = 45.0;

input bool InpUseSpikePredict               = true;
input bool InpUseSpikeConfirm               = true;
input double InpSpikeRangeAtrMult           = 0.60;
input double InpSpikeBodyAtrMult            = 0.25;
input double InpPredictRangeAtrMult         = 0.38;
input double InpPredictBodyAtrMult          = 0.18;
input double InpSpikeMaxWickRatio           = 0.85;

input bool InpUseMomoBreak                  = true;
input double InpMomoRangeAtrMult            = 0.45;
input double InpMomoBodyAtrMult             = 0.20;
...
input bool InpUseLiqFade                    = false;
...
input bool InpUseSqueezeBreak               = false;
input double InpSqueezePct                  = 0.55;

input bool InpUseSessionRangeBreak          = false;
...
input double InpRSIBurstBodyAtr             = 0.60;

input bool InpUseHourBlockMomentum          = false;
input int InpHourBlockStart                 = 13;
input int InpHourBlockEnd                   = 17;

input bool InpUseDualTFImpulse              = true;
input double InpDualM5BodyAtr               = 0.95;
input double InpDualM1BodyAtr               = 0.40;
...
input double InpMaxCandleUsedPct            = 0.50;
input double InpMaxDistFromOpenAtr          = 0.60;
...
int OnInit()
{
   ...
   {
      Print("M722 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M722 ready. M721 + strong spike override.";
   ShowDashboard();
...
void TryOpenTrade()
{
   ...
   if(InpEnforceTrendAlign)
   {
      ...
      bool strongSpike = false;
      if(atr[1] > 0.0)
      {
         const double liveBody = MathAbs(rates[0].close - rates[0].open);
         const double liveRange = rates[0].high - rates[0].low;
         if(liveBody > atr[1] * 1.0 && liveRange > atr[1] * 1.2)
            strongSpike = true;
      }

      if(signal > 0 && !BuyTrendAllowed(trendDir, mid, signalEma[0], m1Slow[0]) && !strongSpike)
      {
      ...
      if(signal < 0 && !SellTrendAllowed(trendDir, mid, signalEma[0], m1Slow[0]) && !strongSpike)
      {
   ...
   if(IsEntryTooLate(signal, atr[1]))
   {
      lastStatus = "Blocked: entry too late (candle move exhausted).";
      lastSignal = StringFormat("path=%s dir=%d – move already done", path, signal);
      return;
   }
...
//+------------------------------------------------------------------+
bool IsEntryTooLate(const int direction, const double atrValue)
{
   if(!InpUseEarlyEntryFilter || atrValue <= 0.0)
      return false;

   MqlRates bar[];
   ArrayResize(bar, 2);
   ArraySetAsSeries(bar, true);
   if(CopyRates(InpTradeSymbol, InpSignalTF, 0, 2, bar) < 1)
      return false;

   const double range = bar[0].high - bar[0].low;
   const double distFromOpen = MathAbs(bar[0].close - bar[0].open);

   if(distFromOpen > atrValue * InpMaxDistFromOpenAtr)
      return true;

   const datetime barTime = bar[0].time;
   const int periodSec = PeriodSeconds(InpSignalTF);
   if(periodSec <= 0)
      return false;

   const double elapsed = (double)(TimeCurrent() - barTime);
   const double pctUsed = elapsed / (double)periodSec;

   if(pctUsed > InpMaxCandleUsedPct && range > atrValue * 0.3)
      return true;

   if(direction > 0)
   {
      if(bar[0].close > bar[0].open && (bar[0].high - bar[0].close) < range * 0.15)
         return true;
   }
   else
   {
      if(bar[0].close < bar[0].open && (bar[0].close - bar[0].low) < range * 0.15)
         return true;
   }
...
{
   return "M722_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M722_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M722 +SpikeOverride ===\n";
