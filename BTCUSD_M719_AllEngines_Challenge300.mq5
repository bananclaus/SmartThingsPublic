//+------------------------------------------------------------------+
//| BTCUSD_M719_AllEngines_Challenge300.mq5                         |
//| M719: Wider trail, trail only after step lock, let winners run
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M719 AllEngines";
input int InpMagicNumber                    = 719;
...
int OnInit()
{
   ...
   {
      Print("M719 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M719 ready. Wide trail, let winners run.";
   ShowDashboard();
...
{
   return "M719_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M719_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M719 Wide Trail + Run ===\n";
