//+------------------------------------------------------------------+
//| BTCUSD_M713_AllEngines_Challenge300.mq5                         |
//| M713: BTC smart entry + momentum ride – better timing & exits
//+------------------------------------------------------------------+
#property strict
...
input string InpBotComment                  = "BTCUSD M713 AllEngines";
input int InpMagicNumber                    = 713;
...
input double InpProfitStepUSD                = 15.0;
...
int OnInit()
{
   ...
   {
      Print("M713 init failed: indicator handle creation failed.");
      return INIT_FAILED;
   }
   ...
   lastStatus = "M713 ready. Smart entry + momentum ride.";
   ShowDashboard();
...
{
   return "M713_" + prefix + "_" + IntegerToString(ticket);
}

//+------------------------------------------------------------------+
string ChartLblName(const string prefix, const ulong ticket)
{
   return "M713_" + prefix + "_LBL_" + IntegerToString(ticket);
}
...
void ShowDashboard()
{
   ...
   const double dayProfit = GetMyDayProfit();
   string text = "";
   text += "=== BTCUSD M713 Smart Entry + Ride ===\n";
